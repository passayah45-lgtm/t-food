...
export default function AddressBook({ defaultName = '', defaultPhone = '' }) {
  ...
  const locate = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      position => {
        ...
        setLocating(false)
        toast.success('Precise location added')
      },
      () => {
        setLocating(false)
  ...
  const submit = async event => {
    ...
    try {
      ...
      toast.error(typeof data === 'string' ? data : data?.non_field_errors?.[0] || data?.detail || 'Could not save address.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const remove = async address => {
    if (!window.confirm(`Delete ${address.label_display} address?`)) return
    try {
  ...
  return (
    <section className="mb-6">
      <div className="flex items-center justify-between gap-4 mb-4">
        <div>
          <h2 className="font-semibold text-gray-950">Saved addresses</h2>
          <p className="text-sm text-gray-500">Choose faster at checkout.</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <form onSubmit={submit} className="card p-5 mb-4 grid sm:grid-cols-2 gap-4">
          ...
          <button type="button" onClick={locate} disabled={locating} className="btn-secondary inline-flex items-center justify-center gap-2">
            <LocateFixed size={16} /> {locating ? 'Locating...' : form.latitude ? 'Location added' : 'Add precise location'}
          </button>
          ...
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setShowForm(false)} className="btn-secondary inline-flex items-center gap-2"><X size={16} /> Cancel</button>
            <button disabled={saving} className="btn-primary inline-flex items-center gap-2"><Save size={16} /> {saving ? 'Saving...' : 'Save'}</button>
          </div>
        </form>
      )}
        ...
        {addresses.map(address => (
          <article key={address.id} className="card p-4">
            ...
            <div className="flex items-center gap-2 mt-4 border-t border-gray-100 pt-3">
              {!address.is_default && <button type="button" onClick={() => makeDefault(address)} className="text-xs font-medium text-brand-600">Set default</button>}
              <button type="button" onClick={() => startEdit(address)} className="ml-auto p-2 text-gray-500 hover:text-gray-900" title="Edit address"><Pencil size={16} /></button>
              <button type="button" onClick={() => remove(address)} className="p-2 text-gray-500 hover:text-red-600" title="Delete address"><Trash2 size={16} /></button>
            </div>
          </article>
        ))}
      </div>
